function VRCursor(mode) {
  ...
  this.ready = new Promise(function(resolve) {
      ...
      // cursor (line)
      var material = new THREE.LineBasicMaterial({color: 0xFF33CC, linewidth: 5});
...
VRCursor.prototype.init = function( dom, camera, context, selectableObjects ) {
  this.dom = dom;
  this.camera = camera;
  this.context = context;
...
VRCursor.prototype.updateCursorIntersection = function() {
  ...
  if (this.selectableObjects !== null) {
    intersects = raycaster.intersectObjects( this.selectableObjects );
  } else {
    intersects = raycaster.intersectObjects( this.context.children );
  }
